...
For later changes, run only `npx wrangler@latest deploy` from this directory. The `assets.directory` setting publishes the repository's `installers/` directory, so changes to `installers/siem.sh` are included automatically. The deployed `*.workers.dev` URL becomes the `SCREAMSIEM_INSTALLER_URL` used by both scripts. Test it with:
